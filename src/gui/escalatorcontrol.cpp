...
EscalatorControl::EscalatorControl(DebugPanel* parent, wxWindowID id)
{
    ...
    Create(parent, id, _("Escalator Control"), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE, _T("id"));
...
void EscalatorControl::BuildList(bool restore_selection)
{
	...
	if (count != lastcount)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (esc)
				ListBox1->Append(wxString(SBS::ToString(i + 1)) + wxT(": ") + esc->GetName());
		}
