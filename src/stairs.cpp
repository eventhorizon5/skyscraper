...
int Stairs::AddStairs(int floor, const char *name, const char *texture, const char *direction, double thickness, double CenterX, double CenterZ, double width, double risersize, double treadsize, int num_stairs, double voffset, double tw, double th, double tw_s, double th_s)
{
	...
	for (int i = 1; i <= num_stairs; i++)
	{
		...
		if (Direction == "right")
		{
			...
			tmpindex = AddWall(floor, buffer.GetData(), texture, thickness, pos + treadsize, -(width / 2), pos + treadsize, width / 2, risersize, risersize, voffset + (risersize * (i - 1)), voffset + (risersize * (i - 1)), tw, th, tw_s, th_s, false, false, false);
			buffer = buffer3 + " " + buffer2 + "-tread";
			if (i != num_stairs)
				AddFloor(floor, buffer.GetData(), texture, thickness, pos, -(width / 2), pos + treadsize, width / 2, voffset + (risersize * i), voffset + (risersize * i), tw, th, tw_s, th_s);
		}
		if (Direction == "left")
		{
			pos = CenterX - ((treadsize * (num_stairs - 1)) / 2) + (treadsize * i);
		...
		if (Direction == "back")
		{
			...
			buffer = buffer3 + " " + buffer2 + "-riser";
			tmpindex = AddWall(floor, buffer.GetData(), texture, thickness, width / 2, pos + treadsize, -(width / 2), pos + treadsize, risersize, risersize, voffset + (risersize * (i - 1)), voffset + (risersize * (i - 1)), tw, th, tw_s, th_s, false, false, false);
			buffer = buffer3 + " " + buffer2 + "-tread";
			if (i != num_stairs)
				AddFloor(floor, buffer.GetData(), texture, thickness, width / 2, pos, -(width / 2), pos + treadsize, voffset + (risersize * i), voffset + (risersize * i), tw, th, tw_s, th_s);
		}
		if (Direction == "front")
		{
			pos = CenterZ - ((treadsize * (num_stairs - 1)) / 2) + (treadsize * i);
...
int Stairs::AddWall(int floor, const char *name, const char *texture, double thickness, double x1, double z1, double x2, double z2, double height1, double height2, double voffset1, double voffset2, double tw, double th, double tw_s, double th_s, bool revX, bool revY, bool revZ)
{
	return sbs->AddWallMain(StairArray_state[floor - startfloor], name, texture, thickness, x1, z1, x2, z2, height1, height2, voffset1, voffset2, tw, th, tw_s, th_s, revX, revY, revZ);
}

int Stairs::AddFloor(int floor, const char *name, const char *texture, double thickness, double x1, double z1, double x2, double z2, double voffset1, double voffset2, double tw, double th, double tw_s, double th_s)
{
	return sbs->AddFloorMain(StairArray_state[floor - startfloor], name, texture, thickness, x1, z1, x2, z2, voffset1, voffset2, tw, th, tw_s, th_s);
}
